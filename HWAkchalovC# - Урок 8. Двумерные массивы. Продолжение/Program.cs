...
int[,,] CreateRandom3dArray()
{
    ...
    for(int i = 0; i < x; i++)
        for(int j = 0; j < y; j++)
            for(int k = 0; k < z; k++)
            {
                Console.Write("Enter a unique two-digit number: ");
                int value = Convert.ToInt32(Console.ReadLine());
                array[i,j,k] = value;// Универсальное решение с неповторяющимися числами пока не сообразил.
            }
    return array;
}
...
// Приветствую, Павел! С Новым Годом, Вас!

// Над 62 задачей буду думать...так же как и над универ-м решением 60 задачи.
// Это действительно интересно, однако времени в обрез, т.к. болел грипом конец декабря и начало января.
// Сейчас, стараюсь решить задачи на "скорую руку" - 
// т.е. не хочется быть отстающим и вовремя проходить уже новый материал с группой.
// К сожалению решения даются мне "непросто"(долго т.к. стараюсь понять процесс, логику), но надеюсь с опытом будет легче.
